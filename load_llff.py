...
def _minify(basedir, factors=[], resolutions=[]):
    """
    Creates new image dirs with resized images based on given factors and resolutions

    Args:
        basedir (str): The base directory containing image directories
        factors (list): Resize factors for new resized images
        resolutiosn (list): Resolutions for new resized images
    """
    needtoload = False
    for r in factors:
    ...
    imgs = [f for f in imgs if any([f.endswith(ex) for ex in ['JPG', 'jpg', 'png', 'jpeg', 'PNG', 'JPEG']])]
    imgdir_orig = imgdir
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, mask_imgs=False):
    ...
    Args:
        basedir (str): The base directory containing image directories
        factor (int, optional): Factor for image resizing. Defaults to None.
        width (int, optional): Width for image resizing. Defaults to None.
        height (int, optional): Heigh for image resizing. Defaults to None.
        load_imgs (bool, optional): Specifies whether to load the images. Defaults to True.

    Returns:
        tuple: Poses, poses bounds, and (optionally) images
    """
...
def normalize(x):
    ...
    Args:
        x (array): The vector to normalize

    Returns:
        array: The normalized vector
    """
    return x / np.linalg.norm(x)


def viewmatrix(z, up, pos):
    """ Get view transform from vectors z and up and position """
    vec2 = normalize(z)
    vec1_avg = up
...
def ptstocam(pts, c2w):
    """ Transform pts from camera to world coordinates """
    tt = np.matmul(c2w[:3,:3].T, (pts-c2w[:3,3])[...,np.newaxis])[...,0]
    return tt

def poses_avg(poses):
    """ Get camera to world transform by averaging camera poses """
    hwf = poses[0, :3, -1:]
...
def spherify_poses(poses, bds):
    ...
    # transform poses from world to camera coordinates
    poses_reset = np.linalg.inv(p34_to_44(c2w[None])) @ p34_to_44(poses[:,:3,:4])

    # calculate mean radius of poses from center
    rad = np.sqrt(np.mean(np.sum(np.square(poses_reset[:,:3,3]), -1)))

    # unit scale everything
    sc = 1./rad
    poses_reset[:,:3,3] *= sc
    bds *= sc
    rad *= sc
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, mask_imgs=False):
    ...
    # recenter poses around average pose
    if recenter:
        poses = recenter_poses(poses)

    # spherify poses for 360 scenes
    if spherify:
